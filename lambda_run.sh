...
if [ -f "$CONFIG_FILE" ]; then
    ...
    if grep -q "from_pretrained.*['\"]\\./ckpts/" "$CONFIG_FILE" 2>/dev/null; then
        ...
        sed -i \
            ...
            -e "s|from_pretrained.*=.*['\"]\\./ckpts/Open_Sora_v2\\.safetensors['\"]|from_pretrained=\"hpcai-tech/Open-Sora-v2/Open_Sora_v2.safetensors\"|g" \
            ...
            -e "s|from_pretrained[[:space:]]*=[[:space:]]*['\"]hpcai-tech/Open-Sora-v2[^'\"]*['\"][[:space:]]*,[[:space:]]*subfolder[[:space:]]*=[[:space:]]*['\"]model['\"]|from_pretrained=\"hpcai-tech/Open-Sora-v2/Open_Sora_v2.safetensors\"|g" \
            "$CONFIG_FILE"
